    ...
    <div class="map-space">
        <div class="left">
            ...
            <div class="li">
                ...
                <div class="input1">
                    <el-select v-model="change_val" placeholder="请选取要绘制数据范围" @change="changeOptions($event)">
            ...
            <div class="li">
                <div class="label">
                    <!--<span class="span2">●</span>-->(转换后:方圆1公里之内):
                </div>
                <div class="input">
            ...
            <div class="li">
                <div class="label">
                    <!--<span class="span3">●</span>-->(转换后:地理编码):
                </div>
                <div class="input">
    ...
    export default {
        ...
        methods: {
            ...
            changeOptions(env) {
                ...
                if (this.markerArr.length != 0 ||this.markerArr1.length != 0 || this.markerArr2.length != 0) {
                    this.removeMarker()
                }
            ...
            getLoactions() {
                var arr = []
                this.locatOptions = ""
                console.log(points_Json.RECORDS.length)
            ...
            btnClickLast() {
                ...
                getLocation(info).then((res) => {
                    ...
                    for (let j = 0; j < pData.length; j++) {
                        ...
                        getLatlng(info1).then(res1 => {
                            //// console.log(res1)
                            for (var key in res1) {
                                var position = new TMap.LatLng(res1.result.location.lat, res1.result
                                    .location.lng);
                                this.markerArr.push({
                                    position: position,
                                    properties: {
                ...
                getLocation(info).then((res) => {
                    ...
                    for (let k = 0; k < latlng.length; k++) { //循环获取经纬度
                        ...
                        getSerchGround(formData).then(res1 => {
                            ...
                            let json_test = {
                                // id:k,
                                '原数据库地址': pData[k].address,
                                // '纬度': pData[k].src_lat,
                                // '经度': pData[k].src_lng,
                                // '数据': res1.data
                            }
                            if (res1.count != 0) {
                                this.json_options1.push(json_test)
                            } else {
                                this.json_options2.push(json_test)
                            }
                            //    console.log([k+3]+pData[k].address+"-"+res1.data[0].location.lat,res1.data[0].location.lng+"---**-"+res1.data[0].address)
                            // console.log(res1.data[0].location.lat,res1.data[0].location.lng)
                            var position = new TMap.LatLng(res1.data[0].location.lat, res1.data[0]
                                .location.lng);
            ...
            //经纬度绘制(转换后-地理编码):
            btnClicPlace() {
                let pData2 = points_Json.RECORDS.slice(this.index_change[0], this.index_change[1])
                for (let j = 0; j < pData2.length; j++) {
                    let formData = {
                        address: pData2[j].address,
                        region: '台州'
                    }
                    getAddress(formData).then(resp => {
